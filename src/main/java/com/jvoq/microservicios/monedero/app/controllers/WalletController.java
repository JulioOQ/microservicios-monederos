...
import com.jvoq.microservicios.monedero.app.services.WalletTransactionService;
...
public class WalletController {
  ...
  public Mono<ResponseEntity<WalletDto>> create(@RequestBody WalletDto walletDto) {
    return walletService.save(walletDto).map(w -> ResponseEntity
        .created(URI.create("/wallets".concat(w.getIdMonedero()))).contentType(MediaType.APPLICATION_JSON).body(w));
  }

  @PutMapping("/{id}")
  public Mono<ResponseEntity<WalletDto>> update(@RequestBody WalletDto walletDto, @PathVariable String id) {
    return walletService.update(walletDto, id).map(w -> ResponseEntity
        .created(URI.create("/wallets".concat(w.getIdMonedero()))).contentType(MediaType.APPLICATION_JSON).body(w))
        .defaultIfEmpty(ResponseEntity.notFound().build());
  }
